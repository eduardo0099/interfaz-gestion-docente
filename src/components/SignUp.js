...
class SignUp extends Component {
	...
	componentDidMount() {
		...
		API.get('/general/listarDepartamentos')
			.then(res => {
	...
	registrarUsuario = (e) => {
		if (this.state.nombres != "" &&
			...
			if (this.state.id_tipo_usuario == 2 || this.state.id_tipo_usuario == 5) {
				...
				console.log("entra en 2x	", parseInt(this.state.id_seccion));
				API.post('auth/register', {
					nombres: this.state.nombres,
					apellido_materno: this.state.apellido_materno,
					apellido_paterno: this.state.apellido_paterno,
	...
	handleChangeVerDep = e => {
		...
		this.setState({ id_departamento: e.target.value });
		API.get('/general/listarSeccionesDep', {
			params: {
				id: e.target.value
			}
		})
				...
				<div className="panel wrapper-md col-lg-offset-1 col-lg-10 col-md-12 col-sm-12">
					...
					<form>
						...
						<FormGroup controlId="formControlTipoDep">
							...
							<option value="0">Seleccionar</option>
								{this.state.listaDeps.map((item, index) => {
									return <option key={index} value={item.id}>{item.nombre}</option>
								})}
							</FormControl>
						</FormGroup>
						{(this.state.listaSecc.length <= 1) && id_tipo_usuario != 3 && id_tipo_usuario != 4?
							<span></span> :
